...
public class AsteroidsManager : MonoBehaviour {
    ...
    private GameObject sampleProjectile = null;
    ...
    void Update ()
    {
        ...
        foreach (GameObject projectile in projectiles)
        {
            for (int i = 0; i < 3; ++i)
            {
                if (projectile.transform.position[i] < -halfExtents[i] || projectile.transform.position[i] > halfExtents[i])
                {
        ...
        foreach (GameObject projectile in toDestroy)
        {
            Destroy(projectile);
        }

        // create new asteroids and place at random places
    ...
    private void shoot()
    {
        GameObject projectile = Instantiate(sampleProjectile);
        projectile.GetComponent<ProjectileController>().setPosition(playerShip.transform.position, playerShip.transform.rotation);
